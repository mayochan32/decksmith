...
`brief.md` は必須ではありません。内容をチャットで直接伝えても大丈夫です。`scene.yaml` や `structure.yaml` はAIが作る内部ファイルなので、自分で書く必要はありません。
...
上の依頼の先頭を `/decksmith` に置き換えて送信します。同じ題材・YAML・参考資料を渡せますが、AIや生成機能が異なれば、構図や画像は同一にはなりません。
...
標準出力はPPTXと `同名.preview/` 内のページ画像です。PDFが必要なら、プレビュー生成時のPDFも納品するよう依頼してください。PDFは標準ではoutput直下へ自動配置されません。
