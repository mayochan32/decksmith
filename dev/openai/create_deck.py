...
from compile_spec import SpecError, compile_scene
...
def main() -> int:
    ...
    parser.add_argument("--scene", required=True, type=Path, help="AIが設計したscene.yaml")
    parser.add_argument("--structure", required=True, type=Path)
    parser.add_argument(
        "--style",
        type=Path,
        required=True,
        help="利用者が指定した元のスタイルYAML。",
    )
    ...
    try:
        spec = compile_scene(args.scene.resolve(), args.style.resolve(), args.structure.resolve())
    except (SpecError, OSError, ValueError) as error:
        parser.error(str(error))
    if args.validate_only:
        print("Scene valid; visual review still required")
        return 0
    ...
    if output.exists() or output.with_suffix(".preview").exists():
    ...
    except (SpecError, OSError, subprocess.CalledProcessError) as error:
